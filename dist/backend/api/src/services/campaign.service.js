...
function mapSmsQueueStateToDispatchState(state, fallbackQueuedAt) {
    return {
...
export class CampaignService {
    ...
    async persistSmsPendingRecipients(campaignId, recipients) {
        const queuedAt = new Date().toISOString();
        await this.app.container.repositories.campaignRecipients.upsertMany(recipients.map((recipient) => ({
            campaignId,
    ...
    buildSmsMockRecipients(message) {
    ...
    async recoverInterruptedSmsDispatch(campaign) {
        if (campaign.channel !== "sms" || campaign.status !== "scheduled")
            return;
    ...
    async create(input) {
        ...
        const estimatedCost = input.channel === "sms"
            ? (await this.app.container.providers.sms.estimateCost({
                recipients: recipients.map((item) => item.phone).filter(Boolean),
    ...
    async resolveRecipients(campaign, recipientIds) {
        ...
        if (recipientIds && recipientIds.length > 0) {
            ...
            const matchedHistoricalRecipients = historicalRecipients.filter((recipient) => (historicalRecipientSet.size === 0 || historicalRecipientSet.has(recipient.legacyId)));
    ...
    async resolveSmsRecipients(campaign, recipientIds) {
    ...
    async executeSend(campaignId, recipientIds, totalRecipientsHint, signal) {
    ...
    async getDispatchStatus(campaignId) {
        ...
        if (state) {
            return campaign.channel === "sms"
                ? mapSmsQueueStateToDispatchState(state, campaign.createdAt)
                : state;
        }
