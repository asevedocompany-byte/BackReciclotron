...
export async function findPontoColetaById(storeid) {
    const pool = getLegacyPool();
    if (!pool)
        return null;
    const [rows] = await pool.query("SELECT * FROM store WHERE storeid = ? LIMIT 1", [storeid]);
    const list = rows;
    return list[0] ?? null;
}
export async function countPontosColeta(filters) {
    const pool = getLegacyPool();
    if (!pool)
        return 0;
    const params = [];
...
//# sourceMappingURL=pontos-coleta.js.map